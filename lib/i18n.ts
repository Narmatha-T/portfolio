...
const jp = {
  ...
  about: {
    ...
    p3: "現在はAkatsuki AI Technologiesにてインド工科大学（IIT）インターン4名からなるグローバルチームと共に、日本の大手クライアントとエンジニアリングチームの技術的橋渡しを担っています。東京在住。英語（ネイティブ）とビジネス日本語（JLPT N3/N2）のバイリンガル環境で日々業務を行っています。",
  },
  skills: {
    label: "01 — 技術スキル",
    heading: "ツール＆技術",
  ...
  projectsData: {
    "01": {
      tagline: "マルチテナント型コンサートアクセスプラットフォーム",
      description:
        "Adoをはじめとするトップアーティスト向けのホワイトラベル型チケット・アクセス管理システム。数万人規模の同時アクセスに対応。Redis Luaスクリプトによるアトミックな在庫管理ロジックで二重予約を物理的に防止し、アーティストごとのマルチテナントアーキテクチャ構築とFigmaによるUI/UXデザインを一貫して担当。",
      category: "バックエンドシステム",
    },
    "02": {
    ...
    "04": {
      ...
      description:
        "飲食店・オフィス・クリニックなどにリアルタイムの混雑状況を提供するシステム。TensorFlow.js（COCO-SSD）によりブラウザ上でリアルタイム人物検出を実現し、高価なGPUリソースが不要。ユーザーが設定した最大収容人数に対するリアルタイム混雑スコアを算出。LINEログインと統合し、オーナーは過去5日間の混雑傾向を確認でき、カスタマイズ可能な埋め込みウィジェットで自サイトへのリアルタイム混雑情報配信も対応。カメラ映像を実用的なデータに変換するエンドツーエンドのソリューション。",
      category: "AIoT",
    },
    "05": {
      tagline: "工業用コンベアベルト上でのAI瓶仕分け",
      description:
        "環境ソリューションを専門とする工場自動化メーカー向けに構築。USBカメラでリサイクルコンベアの映像をリアルタイムキャプチャし、ファインチューニングしたYOLOモデルで透明（白）・茶色ガラス瓶を缶・PETボトル・他色ガラスなどの混合廃棄物から識別・分類。検出座標をOmron PLCに送信し、ロボットアームが対象ボトルを自動でピックアップ・除去するリサイクル工程の全自動化を実現。カスタムアノテーションデータセットを用い、2フェーズに分けて納品。",
      category: "コンピュータービジョン",
    },
    "06": {
    ...
    "09": {
      ...
      description:
        "巡回デバイスの契約モバイルデータ使用量を監視・最適化する自動インフラツール。月次消費量を予測するアルゴリズムを開発し、サービス稼働率100%を維持しながら運用コストを最小化するようモバイルプランを自律的に切り替え（100MB ⇄ 1GB）。LaravelとGoutteで構築し、全デバイスのリアルタイム異常検知と自動監査ログ機能を実装。Laravel Schedulerと連携し、通信量異常の監視とプラン変更のリアルタイムアラートも提供。",
      category: "インフラ自動化",
    },
    "10": {
  ...
  experienceHighlights: [
    [
      "インド工科大学（IIT）の学生4名からなるグローバル開発チームをリードし、本番グレードのコンピュータービジョン・LLM/RAGシステムを全編英語で構築。",
    ...
    [
      "3年間にわたり、通信・リサイクル・公共安全の各分野で8件以上の本番AIシステムを構築・デプロイ。",
      "MEGURU：ナンバープレート検出の処理速度を12.5倍改善、FaceNet512顔認証システムをエンタープライズセキュリティ向けにデプロイ。",
      "壁面劣化分類器：ConvNeXt/Swin-Transformerアンサンブルによりインフラ検査で90%以上の精度を達成。",
      "YOLOXを用いたPETボトルリサイクル分類と、MobileSAMによる倉庫内背景分離システムを開発。",
      "pix2pix＋TextureGAN GANsによる着物テクスチャ生成、COCO-SSD＋LINEログインを活用した混雑監視プラットフォームを構築。",
      "Django・Redis・Celeryを用いた高並列バックエンドシステムを設計し数千の同時ユーザーに対応。TensorFlow.jsによるブラウザサイドAI推論も実装。",
    ],
  ] as string[][],
};
